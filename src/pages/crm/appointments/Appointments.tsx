...
const getTypeStyle = (type: string) => {
  const styles: Record<string, string> = {
    'Fitting': 'bg-[var(--primary-hex)]/10 text-[#6200EA] border-[var(--primary-hex)]/20',
    'Design Consultation': 'bg-[#7A5AA8]/10 text-[#5d4485] border-[#7A5AA8]/20',
    'Fabric Selection': 'bg-[#10B981]/10 text-[#234638] border-[#10B981]/20',
    'Measurement Collection': 'bg-[var(--primary-hex)]/10 text-[#a3531f] border-[var(--primary-hex)]/20',
  };
  return styles[type] || 'bg-[var(--primary-hex)]/10 text-[var(--primary-hex)] border-[var(--primary-hex)]/20';
};

const statusStyles: Record<Appointment['status'], string> = {
  'Scheduled': 'bg-[var(--primary-hex)]/[0.05] text-[var(--primary-hex)]/70',
      ...
      <div className="flex flex-col h-full space-y-5 p-6 md:p-8 max-w-[1500px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {filteredAppointments.map((apt) => (
            <div key={apt.id} className="bg-white p-5 rounded-2xl border border-[var(--primary-hex)]/[0.06] shadow-[0_1px_3px_rgba(28,36,48,0.04)] hover:shadow-[0_8px_18px_rgba(28,36,48,0.08)] hover:-translate-y-0.5 transition-all duration-200 flex flex-col justify-between space-y-4">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <h3 className="text-lg font-serif font-semibold text-[var(--primary-hex)] mt-3">{apt.customerName}</h3>
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-[10px] bg-[var(--primary-hex)]/[0.05] text-[var(--primary-hex)]/55 font-bold uppercase tracking-wider px-2 py-0.5 rounded">
                      {apt.displayId}
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <button 
                    ...
                    className="text-[var(--primary-hex)]/30 hover:text-[#F43F5E] transition p-1"
              ...
              <div className="space-y-2.5 text-xs font-semibold text-[var(--primary-hex)]/55 pt-2">
                <div className="flex items-center gap-2">
                  <CalendarIcon className="w-4 h-4 text-[var(--primary-hex)]/35" />
                  <span>{apt.date}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Clock className="w-4 h-4 text-[var(--primary-hex)]/35" />
                  <span>{apt.time}</span>
                </div>
                {apt.phone && (
                  <div className="flex items-center gap-2">
                    <Phone className="w-4 h-4 text-[var(--primary-hex)]/35" />
                    <span>{apt.phone}</span>
                  </div>
                )}
                {apt.assignedTo && (
                  <div className="flex items-center gap-2">
                    <UserCheck className="w-4 h-4 text-[var(--primary-hex)]/35" />
                    <span>{apt.assignedTo}</span>
                  </div>
                )}
                {apt.notes && (
                  <div className="flex items-start gap-2 pt-3 mt-1 border-t border-[var(--primary-hex)]/[0.04] text-[var(--primary-hex)]/65">
                    <MessageSquare className="w-4 h-4 text-[var(--primary-hex)]/35 mt-0.5 flex-shrink-0" />
              ...
              {apt.status === 'Scheduled' && (
                <div className="flex gap-2 pt-4 mt-2 border-t border-[var(--primary-hex)]/[0.06]">
                  <button 
          ...
          {filteredAppointments.length === 0 && (
             <div className="col-span-full py-12 text-center text-sm font-semibold text-[var(--primary-hex)]/35">
        ...
        {isModalOpen && (
          <div className="fixed inset-0 bg-[var(--primary-hex)]/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-3xl border border-[var(--primary-hex)]/[0.06] shadow-2xl shadow-[var(--primary-hex)]/20 w-full max-w-md overflow-hidden">
              <div className="px-6 py-5 border-b border-[var(--primary-hex)]/[0.08] flex justify-between items-center">
                <div>
                  <p className="text-[10px] font-semibold tracking-[0.18em] uppercase text-[var(--primary-hex)] mb-1">New Entry</p>
                  <h3 className="font-serif font-semibold text-[var(--primary-hex)] text-lg">Book Appointment</h3>
                </div>
                <button 
                  onClick={() => setIsModalOpen(false)}
                  className="text-[var(--primary-hex)]/35 hover:text-[var(--primary-hex)] hover:bg-[var(--primary-hex)]/[0.05] p-1.5 rounded-lg transition"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              <form onSubmit={handleCreateAppointment} className="p-6 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input 
                      ...
                      required
                      className="w-full px-4 py-3 border border-[var(--primary-hex)]/[0.1] rounded-xl focus:outline-none focus:ring-2 focus:ring-[var(--primary-hex)]/25 focus:border-[var(--primary-hex)]/40 text-sm transition text-[var(--primary-hex)]"
                    />
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-[var(--primary-hex)]/45 uppercase tracking-wider mb-1.5">Assigned To</label>
                    <input 
                      type="text" 
                      value={assignedTo} 
                ...
                <div>
                  <label className="block text-xs font-bold text-[var(--primary-hex)]/45 uppercase tracking-wider mb-1.5">Consultation Type</label>
                  <input 
                    type="text" 
                    value={type} 
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    rows={3}
                    className="w-full px-4 py-3 border border-[var(--primary-hex)]/[0.1] rounded-xl focus:outline-none focus:ring-2 focus:ring-[var(--primary-hex)]/25 focus:border-[var(--primary-hex)]/40 text-sm resize-none transition"
                  />
                </div>
                <button 
                  type="submit" 
                  disabled={isSubmitting}
                  className="w-full py-3 bg-[var(--primary-hex)] hover:bg-[#2a3545] disabled:opacity-60 text-[#F4F3F8] rounded-xl text-sm font-semibold transition mt-2 shadow-md shadow-[var(--primary-hex)]/10 flex items-center justify-center gap-2 cursor-pointer"
                >
