...
import { Star, Gift, Search, TrendingUp, Settings } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { reportApi, CustomersReport } from '../../../api/reportApi';
...
const tierConfig: Record<string, { color: string; spendForNext: number; next: string }> = {
  Gold: { color: 'bg-amber-50 text-amber-700 border-amber-100', spendForNext: 50000, next: 'Max Tier' },
  Silver: { color: 'bg-slate-100 text-slate-600 border-slate-200', spendForNext: 50000, next: 'Gold at ₹50K' },
  Regular: { color: 'bg-blue-50 text-blue-700 border-blue-100', spendForNext: 20000, next: 'Silver at ₹20K' },
  New: { color: 'bg-emerald-50 text-emerald-700 border-emerald-100', spendForNext: 1, next: 'Regular on 1st Order' },
};
...
const Loyalty: React.FC = () => {
  ...
  const members: LoyaltyMember[] = data ? data.customers.map(c => ({
    name: c.name,
    points: c.loyaltyPoints || 0,
    tier: c.loyalty || 'New',
    totalSpend: c.totalSpend,
    joinDate: c.lastOrder // Reusing lastOrder as a proxy for now
  })) : [];
  ...
  return (
    <div className="flex flex-col h-full space-y-5 p-6 bg-slate-50/50">
      <div className="flex justify-between items-center pb-4 border-b border-slate-100">
        ...
        <button onClick={() => navigate('/settings/loyalty')} className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg text-sm font-semibold flex items-center gap-1.5 transition shadow-sm">
          <Settings className="w-4 h-4" /> Configure Program
        </button>
      </div>

      {loading ? (
        <div className="flex justify-center items-center h-40 text-slate-500">Loading loyalty data...</div>
      ) : (
        <>
          {/* Summary Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
            <div><span className="text-xs font-bold text-slate-400 uppercase">Total Members</span><h3 className="text-2xl font-black text-slate-900 mt-1">{members.length}</h3></div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
            <div><span className="text-xs font-bold text-slate-400 uppercase">Gold Members</span><h3 className="text-2xl font-black text-slate-900 mt-1">{members.filter(m => m.tier === 'Gold').length}</h3></div>
      ...
      <div className="flex-1 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <table className="w-full text-left text-sm text-slate-600">
          ...
          <tbody className="divide-y divide-slate-50">
            {filtered.map(m => {
              const cfg = tierConfig[m.tier] || tierConfig['New'];
              const pct = m.tier === 'Gold' ? 100 : Math.min(100, Math.round((m.totalSpend / cfg.spendForNext) * 100));
              return (
                <tr key={m.name} className="hover:bg-slate-50/40 transition">
