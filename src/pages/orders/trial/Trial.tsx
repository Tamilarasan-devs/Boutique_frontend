...
const statusStyles: Record<string, string> = {
  ...
  'Alterations Needed': 'bg-[#8338EC]/10 text-[#6200EA]',
    ...
    <div className="min-h-screen bg-[#F4F3F8] text-[#16132D]">
      <div className="flex flex-col h-full space-y-5 p-6 md:p-8 max-w-[1500px] mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 pb-5 border-b border-[#16132D]/[0.08]">
          <div>
            <p className="text-xs font-semibold tracking-[0.18em] uppercase text-[#7209B7] mb-1.5">Fittings</p>
            <h1 className="text-3xl md:text-[2rem] font-serif font-semibold tracking-tight text-[#16132D]">Trial Sessions</h1>
            <p className="text-sm text-[#16132D]/55 mt-1">Schedule fittings and capture alteration feedback from customers.</p>
          </div>
        ...
        <div className="flex items-center bg-white border border-[#16132D]/[0.08] rounded-xl px-4 py-2.5 w-full sm:w-80 shadow-sm focus-within:ring-2 focus-within:ring-[#7209B7]/25 transition">
          ...
          <input type="text" placeholder="Search trials..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-transparent border-none outline-none text-sm text-[#16132D] placeholder-[#16132D]/35 w-full" />
        </div>

        {/* Cards Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {filtered.map(trial => (
            <div key={trial.id} className="bg-white p-5 rounded-2xl border border-[#16132D]/[0.06] shadow-[0_1px_3px_rgba(28,36,48,0.04)] hover:shadow-[0_8px_18px_rgba(28,36,48,0.08)] hover:-translate-y-0.5 transition-all duration-200 space-y-4">
              <div className="flex justify-between items-start">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button onClick={() => handleDelete(trial.id)} className="p-1 text-[#16132D]/30 hover:text-[#F43F5E] transition">
                    <Trash2 className="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>

              <div className="flex flex-wrap items-center gap-4 text-xs font-semibold text-[#16132D]/55">
                <span className="flex items-center gap-1.5"><CalendarIcon className="w-3.5 h-3.5 text-[#16132D]/35" /> {trial.date}</span>
                <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-[#16132D]/35" /> {trial.time}</span>
                {trial.tailor && <span className="text-[#16132D]/45">Tailor: <span className="text-[#16132D]">{trial.tailor}</span></span>}
              </div>

              {trial.alterationNotes && (
                <div className="flex items-start gap-2 bg-[#F4F3F8] p-3 rounded-xl border border-[#16132D]/[0.04] text-xs text-[#16132D]/65">
                  <MessageSquare className="w-3.5 h-3.5 text-[#16132D]/35 mt-0.5 flex-shrink-0" />
                  <p className="leading-relaxed">{trial.alterationNotes}</p>
                </div>
              )}

              {trial.status === 'Scheduled' && (
                <div className="flex gap-2 pt-3 border-t border-[#16132D]/[0.06]">
                  ...
                  <button onClick={() => handleUpdateStatus(trial.id, 'Alterations Needed')} className="flex-1 py-2 bg-[#8338EC]/10 hover:bg-[#8338EC]/20 text-[#6200EA] text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 transition">
          ...
          {filtered.length === 0 && (
            <div className="col-span-full py-12 text-center text-sm font-semibold text-[#16132D]/35">No trials found.</div>
          )}
        </div>
          ...
          <div className="fixed inset-0 bg-[#16132D]/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-3xl border border-[#16132D]/[0.06] shadow-2xl shadow-[#16132D]/20 w-full max-w-lg overflow-hidden max-h-[90vh] flex flex-col">
              <div className="px-6 py-5 border-b border-[#16132D]/[0.08] flex justify-between items-center shrink-0">
                <h2 className="text-xl font-serif font-bold text-[#16132D]">Schedule Trial</h2>
                ...
                <form id="trialForm" onSubmit={handleCreate} className="space-y-5">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" value={orderId} onChange={(e) => setOrderId(e.target.value)} placeholder="e.g. ORD-2026-002" className="w-full px-4 py-3 border border-[#16132D]/[0.1] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#7209B7]/25 text-sm transition" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-[#16132D]/45 uppercase tracking-wider mb-1.5">Tailor</label>
                      <input type="text" value={tailor} onChange={(e) => setTailor(e.target.value)} placeholder="e.g. Madan Lal" className="w-full px-4 py-3 border border-[#16132D]/[0.1] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#7209B7]/25 text-sm transition" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-[#16132D]/45 uppercase tracking-wider mb-1.5">Customer Name *</label>
                    <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} required placeholder="e.g. Priyanka Sen" className="w-full px-4 py-3 border border-[#16132D]/[0.1] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#7209B7]/25 text-sm transition" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-[#16132D]/45 uppercase tracking-wider mb-1.5">Garment *</label>
                    <input type="text" value={garment} onChange={(e) => setGarment(e.target.value)} required placeholder="e.g. Designer Lehenga Choli" className="w-full px-4 py-3 border border-[#16132D]/[0.1] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#7209B7]/25 text-sm transition" />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-[#16132D]/45 uppercase tracking-wider mb-1.5">Date *</label>
                      <input type="date" value={date} onChange={(e) => setDate(e.target.value)} required className="w-full px-4 py-3 border border-[#16132D]/[0.1] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#7209B7]/25 text-sm transition" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-[#16132D]/45 uppercase tracking-wider mb-1.5">Time *</label>
                      <input type="time" value={time} onChange={(e) => setTime(e.target.value)} required className="w-full px-4 py-3 border border-[#16132D]/[0.1] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#7209B7]/25 text-sm transition" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-[#16132D]/45 uppercase tracking-wider mb-1.5">Alteration Notes</label>
                    <textarea value={alterationNotes} onChange={(e) => setAlterationNotes(e.target.value)} placeholder="e.g. Customer wants to check choli fitting..." rows={2} className="w-full px-4 py-3 border border-[#16132D]/[0.1] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#7209B7]/25 text-sm resize-none transition" />
                  </div>
                </form>
              </div>
